    ...
    <div className="ai-result-card">
      ...
      {expanded && (
        <>
          ...
          <div style={{ marginTop: '0.75rem' }}>
            <button className="btn btn-secondary btn-sm" onClick={fetchMoreQuestions} disabled={loadingQ}>
              {loadingQ
...
export default function AIResults({ data, jobRequirements }) {
  ...
  // Error from backend (rate limit, API failure etc.)
  if (data.error) {
    return (
      <div className="card" style={{ borderColor: '#f87171' }}>
        <div className="card-title"><Bot size={18} color="#f87171" /> AI Shortlisting Error</div>
    ...
    <div>
      <div className="card">
        <div className="card-title">
          ...
          {data.model && (
            <span style={{ marginLeft: 'auto', fontSize: '0.7rem', color: '#64748b', fontWeight: 400 }}>
              {data.model}
            </span>
          )}
        </div>
        ...
        {data.results && data.results.length > 0 ? (
          data.results.map((result, i) => (
            <CandidateAICard key={i} result={result} index={i} jobRequirements={jobRequirements} />
          ))
        ) : (
          <div className="empty-state">
            <div className="empty-state-icon">🤖</div>
            <div>No candidates matched the job requirements</div>
            <div style={{ fontSize: '0.8rem', marginTop: '0.5rem', color: '#475569' }}>
              Try lowering the minimum experience or adding more candidates
            </div>
          </div>
        )}
      </div>
